...
def _check_managed_local_engine(should_fix: bool, f: Finding) -> None:  # noqa: ARG001
    ...
    from hermes_cli.vllm_runtime.device import CPU, is_vllm_engine
    ...
    if smi:
        ...
        check_warn("nvidia-smi", "not on PATH (GPU vLLM needs an NVIDIA driver)")
        if engine == "vllm":
            f.manual_issues.append("nvidia-smi not found — GPU vLLM needs an NVIDIA GPU")

    for label, leaf, dev in (("vLLM GPU venv", "runtimes/vllm/.venv", "gpu"),
                             ("vLLM CPU venv", "runtimes/vllm-cpu/.venv", "cpu")):
        if venv_ready(dev):
            check_ok(label, f"isolated {leaf}")
        elif is_vllm_engine(engine) and device == dev:
            check_warn(label, "missing — Desktop Local Models → Install")
            f.issues.append(
                f"{label} missing — install from Desktop Settings → Providers → Local Models")
