...
export const zhHant = defineLocale({
  ...
  settings: {
    ...
    localModels: {
      ...
      vramFree: free => `空閒 ${free}`,
      vramEngine: label => `本引擎 ${label}`,
      vramOther: label => `其他程序 ${label}`,
      gpuDriver: version => `驅動 ${version}`,
      ...
      downloaded: '已下載',
      downloadAction: size => (size && size !== '—' ? `下載 · ${size}` : '下載'),
      downloadBare: '下載',
      ...
      vllmReadyTitle: 'vLLM 執行環境已安裝',
      vllmReadyDetail: model =>
        `${model} 已就緒。伺服器由 Hermes 啟動並管理。先下載模型，再按「使用」設為新對話的預設模型。`,
      vllmUseAction: '使用',
      ...
      vllmVersionMissing: '尚未安裝 vLLM。',
      vllmVersionDetail: ver => `正在執行 vLLM ${ver}。`,
      vllmCheckUpdate: '檢查更新',
      vllmCheckingUpdate: '正在檢查…',
      vllmUpdateAvailable: (latest, current) =>
        `有更新的 vLLM 組建（${latest}）可安裝——目前為 ${current}。下載期間模型仍可使用。`,
      vllmUpToDate: ver => `正在執行 vLLM ${ver}——已是 PyPI 上的最新版本。`,
      vllmCheckFailed: '無法檢查 vLLM 引擎更新',
      vllmVenvDetail: path => `安裝位置 ${path}`,
