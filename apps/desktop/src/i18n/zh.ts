...
export const zh: Translations = {
  ...
  settings: {
    ...
    localModels: {
      ...
      vram: label => `${label} 显存`,
      vramUsed: (used, total) => `已用 ${used} / ${total}`,
      vramFree: free => `空闲 ${free}`,
      vramEngine: label => `本引擎 ${label}`,
      vramOther: label => `其他进程 ${label}`,
      gpuDriver: version => `驱动 ${version}`,
      gpuCompute: cap => `算力 ${cap}`,
      modelsPath: '模型缓存',
      ...
      downloaded: '已下载',
      downloadAction: size => (size && size !== '—' ? `下载 · ${size}` : '下载'),
      downloadBare: '下载',
      ...
      vllmReadyTitle: 'vLLM 运行时已安装',
      vllmReadyDetail: model =>
        `${model} 已就绪。服务器由 Hermes 启动并管理。先下载模型，再点「使用」设为新对话的默认模型。`,
      vllmUseAction: '使用',
      ...
      vllmVersionMissing: '尚未安装 vLLM。',
      vllmVersionDetail: ver => `正在运行 vLLM ${ver}。`,
      vllmCheckUpdate: '检查更新',
      vllmCheckingUpdate: '正在检查…',
      vllmUpdateAvailable: (latest, current) =>
        `有更新的 vLLM 构建（${latest}）可安装——当前为 ${current}。下载期间模型仍可使用。`,
      vllmUpToDate: ver => `正在运行 vLLM ${ver}——已是 PyPI 上的最新版本。`,
      vllmCheckFailed: '无法检查 vLLM 引擎更新',
      vllmVenvDetail: path => `安装位置 ${path}`,
