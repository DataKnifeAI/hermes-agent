...
describe('vLLM engine', () => {
  ...
  it('shows a running vLLM pane with Local inventory, not GGUF widgets', async () => {
    ...
    const updateActions = turnOff.closest('.flex')
    expect(updateActions).toBeTruthy()
    expect(within(updateActions as HTMLElement).getByRole('button', { name: /check for update/i })).toBeTruthy()
    expect(updateActions!.className.split(/\s+/)).toEqual(
  ...
  it('shows the llama.cpp engine-update icons for vLLM idle / available / current', async () => {
    ...
    const updateActions = turnOn.closest('.flex')
    expect(updateActions).toBeTruthy()
    expect(within(updateActions as HTMLElement).getByRole('button', { name: /check for update/i })).toBeTruthy()
    expect(updateActions!.className.split(/\s+/)).toEqual(
      expect.arrayContaining(['flex', 'items-center', 'justify-end', 'gap-2'])
